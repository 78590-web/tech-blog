...
既然输出重定向来自于我们的命令的输出结果给到哪里，这是一个问题。那么我们的命令来自于哪里呢，也就是输入的重定向，有了输出的重定向不能没有输入的重定向吧。我们想一想就可以发现，平常输入的时候我们采用的就是键盘，比如你现在打字或者说鼠标滚轮等等，这些都是输入设备，输入设备把信号与指令输入给计算机。所以输入重定向就可以理解为把输入设备换掉，既然我们不输入了，那就是电脑内部的文件进行输入，这样我们就可以从某个文件中读取数据并且进行某些操作。
